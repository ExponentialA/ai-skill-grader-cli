...
```bash
node "$(cat "${CODEX_HOME:-$HOME/.codex}/skill-grader-root")/scripts/report.js" "<skill name or GitHub URL>"
```
...
- If the `skill-grader-root` file does not exist, the grader hasn't run yet this session — start a new Codex session (the SessionStart hook records it) and try again.
