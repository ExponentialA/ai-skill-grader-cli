#!/usr/bin/env node
// Fetches the full report for a skill/repo from AI Skill Grader and prints it,
// using the site's own renderer (reportText), so the content is identical to the
// site and the emailed report. Instant for an already-graded skill; a new one
// comes back as the fast preview, with the deep report available on request by
// email (it is not produced here).
//
// Windsurf build: report-only. Windsurf's hooks can't surface a read (no
// session-start event, no context/message channel), so there is no automatic
// read here — this is the on-demand "full report on X" path. Resolves a bare
// skill name against Windsurf's skill locations (.windsurf/skills, .agents/skills,
// ~/.agents/skills), falling back to the other tools' locations.
const fs = require("fs");
const os = require("os");
...
function skillRoots() {
  ...
  const project = process.env.ROOT_WORKSPACE_PATH || process.cwd();
  return [
