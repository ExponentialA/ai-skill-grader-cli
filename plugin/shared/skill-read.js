...
function skillLine(skill) {
  ...
  if (sig.manipulation) {
    return `⛔ ${name} — manipulation/exfiltration language in its SKILL.md. I'll block its scripts from running; read ${file} before you trust it.`;
  }
  ...
  return `✓ ${name} — no run-safety flags, safe to inspect.`;
}
