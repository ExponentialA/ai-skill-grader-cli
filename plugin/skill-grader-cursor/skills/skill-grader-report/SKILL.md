...
```bash
node "$(cat "$HOME/.cursor/skill-grader-root")/scripts/report.js" "<skill name or GitHub URL>"
```
...
- If the `skill-grader-root` file does not exist, the grader hasn't run yet this session — start a new Cursor session (the sessionStart hook records it) and try again.
